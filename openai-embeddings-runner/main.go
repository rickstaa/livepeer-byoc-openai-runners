...
func main() {
	...
	if upstream == "" {
		log.Fatal("UPSTREAM_URL is required, e.g. http://HOST:PORT/v1/embeddings")
	}
	...
	mux.HandleFunc("/v1/embeddings", func(w http.ResponseWriter, r *http.Request) {
		...
		// Also do not forward any user Authorization
		...
		// Copy response headers/status and stream body
	...
	log.Printf("BYOC embeddings runner listening on %s, upstream=%s", addr, upstream)
	srv := &http.Server{
		Addr:              addr,
		Handler:           mux,
